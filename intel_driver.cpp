...
HANDLE intel_driver::Load() {
	...
	std::string win32k_path = std::string(systemRootEnv) + "\\win32k.sys";
	ez::pdb win32k_pdb = ez::pdb(win32k_path);
	if (!win32k_pdb.init())
	{
	...
	if (!utils::GetVersion(&osvi))
	{
		Log(L"[-] Failed to get OS version information." << std::endl);
		intel_driver::Unload(result);
		return INVALID_HANDLE_VALUE;
	}
	if (osvi.dwBuildNumber >= 26100) {
		int gSessionGlobalSlots_rva = win32k_pdb.get_rva("gSessionGlobalSlots");
		...
		uintptr_t nt_qword = GetSessionState24H2 + 0x88;
		...
		NtUserSetGestureConfig_ref = nt_qword + 0xC30;
	}
	else
	{
		int NtUserSetGestureConfig_rva = win32k_pdb.get_rva("NtUserSetGestureConfig");
