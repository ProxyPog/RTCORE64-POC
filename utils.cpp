...
BOOL utils::GetVersion(PRTL_OSVERSIONINFOW pOsVersionInfo) {
	...
	pOsVersionInfo->dwOSVersionInfoSize = sizeof(RTL_OSVERSIONINFOW);

	if (RtlGetVersion(pOsVersionInfo) == 0) { // STATUS_SUCCESS
		return TRUE;
	}
	else {
